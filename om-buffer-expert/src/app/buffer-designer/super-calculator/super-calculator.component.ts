...
export class SuperCalculatorComponent implements OnInit, OnDestroy {
  ...
  pH_adjusting_compound_options: string[] = [
    'Auto',
    'Sodium Hydroxide',
    'Potassium Hydroxide',
    'Hydrochloric Acid',
  ...
  ngOnInit(): void {
    ...
    this.all_buffer_compound_names =
      this.solutionMixtureService.buffer_compounds;
 ...
 this.submitted = false;
    ...
    this.bufferForm.get('buffer_compound3')?.valueChanges.subscribe({
      next: (value) => {
        this.second_compound_options = this.all_buffer_compound_names
          .filter((compound) => compound !== value)
          .sort();
 ...
 this.submitted = false;
  ...
  populateForm(solution: Solution): void {
    ...
    if (solution) {
      this.bufferForm
        .get('buffer_compound1')
        ?.setValue(solution.non_salt_compounds[0].name);
      this.bufferForm
        .get('buffer_compound2')
        ?.setValue(solution.non_salt_compounds[1].name);
      this.bufferForm
        .get('concentration1')
        ?.setValue(
          solution.compound_concentrations[
            solution.non_salt_compounds[0].name
          ].toPrecision(3)
        );
      this.bufferForm
        .get('concentration2')
      ...
      if (solution.non_salt_compounds.length == 3) {
        this.bufferForm
          ...
          ?.setValue(solution.non_salt_compounds[2].name);
        this.bufferForm
          .get('concentration3')
          ?.setValue(
            solution.compound_concentrations[
  ...
  onSubmit(): void {
    ...
    ['buffer_compound1', 'buffer_compound2', 'buffer_compound3'].forEach(
      (compound, i) => {
        if (this.bufferForm.get(compound).value !== 'None') {
          data.compound_names.push(this.bufferForm.get(compound).value);
          data.compound_concs.push(
            parseFloat(this.bufferForm.get(`concentration${i + 1}`).value)
          );
        }
      }
    );
    ...
    if (this.bufferForm.get('salt_compound').value !== 'None') {
      ...
      data.salt_conc = parseFloat(
        this.bufferForm.get('salt_concentration').value
      );
    }
